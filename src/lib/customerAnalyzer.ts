...
export function analyzeKeyCustomers(
  inputText: string,
  pplRecords: PPLRecord[],
  ...
  const matchedNames = new Set(
    matchResults
      ...
      .map((item) => normalizeCustomerName(item.matchedCustomerName)),
  );
  const matchedPplRows = pplRecords.filter((row) =>
    matchedNames.has(normalizeCustomerName(row.customerName)),
  );
  ...
  return {
    ...
    kpis: {
      ...
      matchedCustomerCount: new Set(
        matchResults.filter((item) => item.matched).map((item) => item.matchedCustomerName),
      ).size,
      opportunityCount: matchedPplRows.length,
      totalAmount,
      weightedWinRate: totalAmount
        ? sum(matchedPplRows.map((row) => row.amount * row.winRate)) / totalAmount
        : 0,
      forecastAmount: sum(
        matchedPplRows
